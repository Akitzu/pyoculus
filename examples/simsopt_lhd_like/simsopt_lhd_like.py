...
# Here the magnetic axis is found when initializing the problem using a guess
...
# fp_x1.compute(guess=[4.8827, 0.], pp=1, qq=2, sbegin=2, send=5.5, checkonly=True)

# Island m = 7, n = ? (10 ?)
fp_7o = FixedPoint(pyoproblem, pparams, integrator_params=iparams)
fp_7o.compute(guess=[4.5394, 0.0], pp=1, qq=7, sbegin=2, send=5.5, checkonly=True)
fp_7x = FixedPoint(pyoproblem, pparams, integrator_params=iparams)
fp_7x.compute(
    guess=[4.45012135, -0.15772282], pp=1, qq=7, sbegin=2, send=5.5, checkonly=True
)

# Island m = 13, n = ? (20 ?)
fp_13o = FixedPoint(pyoproblem, pparams, integrator_params=iparams)
fp_13o.compute(
    guess=[4.54492985, -0.07441066], pp=1, qq=13, sbegin=2, send=5.5, checkonly=True
)
fp_13x = FixedPoint(pyoproblem, pparams, integrator_params=iparams)
fp_13x.compute(guess=[4.56964356, 0.0], pp=1, qq=13, sbegin=2, send=5.5, checkonly=True)

# Island m = 6, n = ? (10 ?)
fp_6o = FixedPoint(pyoproblem, pparams, integrator_params=iparams)
fp_6o.compute(
    guess=[4.49199436, -0.1779669], pp=1, qq=6, sbegin=2, send=5.5, checkonly=True
)
fp_6x = FixedPoint(pyoproblem, pparams, integrator_params=iparams)
fp_6x.compute(guess=[4.60062493, 0.0], pp=1, qq=6, sbegin=2, send=5.5, checkonly=True)

# Island m = 19, n = ? (30 ?)
fp_19o = FixedPoint(pyoproblem, pparams, integrator_params=iparams)
fp_19o.compute(guess=[4.58393511, 0.0], pp=1, qq=19, sbegin=2, send=5.5, checkonly=True)
fp_19x = FixedPoint(pyoproblem, pparams, integrator_params=iparams)
fp_19x.compute(
    guess=[4.5757102, -0.03811962], pp=1, qq=19, sbegin=2, send=5.5, checkonly=True
)
...
for ii, fp in enumerate([fp_7o, fp_13o, fp_6o, fp_19o]):
    ...
    for rr in results:
        axs[1, 0].scatter(
            rr[0], rr[2], marker="o", edgecolors="black", linewidths=1, color=colors[ii]
        )
...
for ii, fp in enumerate([fp_7x, fp_13x, fp_6x, fp_19x]):
    ...
    for rr in results:
        axs[1, 0].scatter(
            rr[0], rr[2], marker="X", edgecolors="black", linewidths=1, color=colors[ii]
        )
proc0_print(
